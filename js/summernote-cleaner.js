    ...
    'cleaner': function (context) {
      ...
      var cleanText = function (txt, nlO) {
        ...
        if (options.cleaner.keepHtml) {
          // Removes the annoying small 'dot' when pasting lists.
          out = out.replace(/<\!\[if.*?\]>.*?<!\[.*?\]>/gi, '');
          // When you are dealing with the Cleaner button the contents is different, we must remove the spans associated with the list.
          //out = out.replace(/<p\s+style=('|"|\\"|\\')text-indent:.*?mso-list:.*?('|"|\\"|\\')>/gi, '<p>');
          out = out.replace(/<p\s+style=('|"|\\"|\\')text-indent:.*?mso-list:.*?('|"|\\"|\\')><span style=('|"|\\"|\\')font-family:Symbol.*?<\/span><\/span><\/span>/gi, '<p>')
